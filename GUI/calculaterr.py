    ...
    try:
        if "^" in expression:
            if "^" == expression[-1]:
                ...
                new_expression = "(" + "(" + expression[0][-1] + ")" + "**" + "2" + ")" + expression[1][
                    0] + "(" + expression[-1:] + ")"
                print(new_expression)
                expression = new_expression
                total = str(eval(expression))
...
def m_plus():
    try:
        ...
        messagebox.showerror(top, "There is Nothing to add in the memory!")
...
def m_sub():
    try:
        ...
        messagebox.showerror(top, "There is Nothing to subtract in the memory!")
...
def m_recall():
    try:
        ...
        messagebox.showerror(top, "There is Nothing to recall in the memory!")
...
def memory():
    ...
    def menu():
            return Menubutton(top2, text="", width=40, bg="white", activebackground="grey")
    ...
    if o2 == []:
        ...
        for j in range(len(o2)):
            b.append(menu())
            b[j].config(text=o2[j])
            options = Menu(b[j])
            b[j].config(menu=options)
            options.add_command(label='MC',  command=lambda id=j: clear(id))
            options.add_command(label='M+', command=lambda id=j: addition(id))
            options.add_command(label='M-', command=lambda id=j: subtract(id))
            b[j].pack(side=BOTTOM)
        top2.resizable(height=False, width=False)
        top2.mainloop()
...
def add1():
    ...
    a = L["text"]
    try:
        if a[29] == "1" or a[29] == "2" or a[29] == "3" or a[29] == "4" or a[29] == "5" or a[29] == "6" or a[29] == "7" \
                or a[29] == "8" or a[29] == "9" or a[29] == "10":
...
button_memory1.config(text="MS", width=10, command=add1)
button_memory1.place(x=90, y=147)

button_memory2 = button()
button_memory2.config(text="M-", width=10, command=m_sub)
button_memory2.place(x=170, y=147)

button_memory3 = button()
button_memory3.config(text="MR", width=10, command=m_recall)
button_memory3.place(x=250, y=147)

button_memory4 = button()
button_memory4.config(text="MC", width=10, command=m_clear)
