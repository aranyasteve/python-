...
def click(row, col):
    ...
    try:
        ...
        for i in range(row1):
            for j in range(col1):
                if b1[i][j]["activebackground"] != "grey" and b1[i][j]["state"] != "disabled":
...
def click1(row, col, num):
    if num == 0:
        b1[row][col].config(activebackground="white", state=NORMAL)
        click(row, col)
        if b1[0][0]["state"] == NORMAL:
            b1[0][0].config(command=lambda row=i, col=j: click1(row, col , 1))
        else:
            if b1[1][0]["state"]== "Normal":
                b1[1][0].config(command=lambda row=i, col=j: click1(row, col, 1))
        # b1[row][col].config(command=lambda row=i, col=j: click1(row, col, 1))
    else:
        global e
        e = e+1
        for row in range(10):
